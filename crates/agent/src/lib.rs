...
pub use r#loop::{Agent, PermissionCallback};
...
pub fn make_agent_tool(
    ...
    let spawn_fn = Arc::new(move |task: String, working_dir: PathBuf| {
        ...
        Box::pin(async move {
            spawn_subagent(provider, vec![], config, working_dir, system_prompt, task).await
...
pub async fn build_system_prompt(config: &Config, working_dir: &PathBuf) -> String {
