...
- Any `in_progress` items in the active [task plan](/tools/task-management) are finalised to `completed` and the plan is saved to disk, so the next run does not resume with stale state

Plan finalisation runs on every exit path from the loop — user cancellation, API errors, `max_tokens`, and normal completion — not only on `Ctrl+C`.
...
- **Active task plan**: The current per-project [plan](/tools/task-management), rendered under an `## Active Task Plan (<completed>/<total> completed, <remaining> remaining)` header grouped by priority. The plan is reloaded from disk and re-injected **before every turn**, so each LLM call sees the live state of the task list without the model having to repeat it in the conversation.
